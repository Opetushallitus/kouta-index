...
(deftest filtered-koulutus-list-test
  (prepare-elastic-test-data)
    (testing "Filter koulutus"
      ...
      (testing "by oid"
        (let [oids (post-200-oids defaultKoulutusOids (str "?nimi=" koulutusOid2))]
          (is (= [koulutusOid2] oids))))
      (testing "by muokkaajan oid"
        (let [oids (post-200-oids defaultKoulutusOids "?muokkaaja=1.2.246.562.24.55555555555")]
          (is (= [koulutusOid3] oids))))
      (testing "by tila"
        (let [oids (post-200-oids defaultKoulutusOids "?tila=tallennettu")]
          (is (= [koulutusOid5] oids))))
      (testing "monella tilalla"
        (let [oids (post-200-oids defaultKoulutusOids "?tila=tallennettu,julkaistu&order-by=tila")]
          (is (= [koulutusOid2 koulutusOid3 koulutusOid5] oids))))
      (testing "monella parametrilla"
        (let [oids (post-200-oids defaultKoulutusOids "?tila=julkaistu&muokkaaja=1.2.246.562.24.55555555555")]
          (is (= [koulutusOid3] oids)))))

    (testing "Sort koulutus result"
      (testing "by tila asc"
        (let [oids (post-200-oids defaultKoulutusOids "?order-by=tila&order=asc")]
          (is (= [koulutusOid4 koulutusOid2 koulutusOid3 koulutusOid5] oids))))
      (testing "by tila desc"
        (let [oids (post-200-oids defaultKoulutusOids "?order-by=tila&order=desc")]
          (is (= [koulutusOid5 koulutusOid2 koulutusOid3 koulutusOid4] oids))))
      (testing "by modified asc"
        (let [oids (post-200-oids defaultKoulutusOids "?tila=julkaistu&order-by=modified&order=asc")]
          (is (= [koulutusOid2 koulutusOid3] oids))))
      (testing "by modified desc"
        (let [oids (post-200-oids defaultKoulutusOids "?tila=julkaistu&order-by=modified&order=desc")]
          (is (= [koulutusOid3 koulutusOid2] oids))))
      (testing "by nimi asc"
        (let [oids (post-200-oids defaultKoulutusOids "?tila=julkaistu&order-by=nimi&order=asc")]
          (is (= [koulutusOid2 koulutusOid3] oids))))
      (testing "by desc desc"
        (let [oids (post-200-oids defaultKoulutusOids "?tila=julkaistu&order-by=nimi&order=desc")]
          (is (= [koulutusOid3 koulutusOid2] oids))))
      (comment testing "by toteutus count asc"
        (let [oids (post-200-oids defaultKoulutusOids "?order-by=toteutukset&order=asc")]
          (is (= [koulutusOid5 koulutusOid3 koulutusOid4 koulutusOid2] oids))))
      (comment testing "by toteutus count desc"
        (let [oids (post-200-oids defaultKoulutusOids "?order-by=toteutukset&order=desc")]
      ...
      (comment testing "by muokkaaja asc"                 ;TODO: muokkaajan nimi onr:stä / nimen mockaus
        (let [oids (post-200-oids defaultKoulutusOids "?tila=julkaistu&order-by=muokkaaja&order=asc")]
          (is (= [koulutusOid2 koulutusOid3] oids))))
      (comment testing "by muokkaaja desc"                ;TODO: muokkaajan nimi onr:stä / nimen mockaus
        (let [oids (post-200-oids defaultKoulutusOids "?tila=julkaistu&order-by=muokkaaja&order=desc")]
          (is (= [koulutusOid3 koulutusOid2] oids)))))

    (testing "Page koulutus result"
      (testing "return first page"
        (let [oids (post-200-oids defaultKoulutusOids "?page=1&size=2&order-by=tila")]
          (is (= [koulutusOid4 koulutusOid2] oids))))
      (testing "return first page when negative page"
        (let [oids (post-200-oids defaultKoulutusOids "?page=-1&size=2&order-by=tila")]
          (is (= [koulutusOid4 koulutusOid2] oids))))
      (testing "return second page"
        (let [oids (post-200-oids defaultKoulutusOids "?page=2&size=2&order-by=tila")]
          (is (= [koulutusOid3 koulutusOid5] oids))))
      (testing "return empty page when none left"
        (let [oids (post-200-oids defaultKoulutusOids "?page=3&size=2&order-by=tila")]
          (is (= [] oids)))))

    (testing "Koulutus result contain proper fields"
      (let [res (post-200 defaultKoulutusOids (str "?nimi=" koulutusOid2))]
        (is (= 1 (:totalCount res)))
    ...
    (comment testing "Filter koulutus by nimi"              ;TODO Tämä eivät saisi feilata!
             (testing "by 'tieto'"
               (let [oids (post-200-oids defaultKoulutusOids "?nimi=tieto")]
                 (is (= [koulutusOid2 koulutusOid3 koulutusOid4 koulutusOid5] oids))))
             (testing "by 'tietojenkäsittely'"
               (let [oids (post-200-oids defaultKoulutusOids "?nimi=tietojenkäsittely")]
                 (is (= [koulutusOid2 koulutusOid4 koulutusOid5] oids))))
             (testing "by 'tietojenkäsittelytiede'"
               (let [oids (post-200-oids defaultKoulutusOids "?nimi=tietojenkäsittelytiede")]
                 (is (= [koulutusOid2 koulutusOid4 koulutusOid5] oids))))
             (testing "by 'tietojenkäsittelytieteen perusopinnot'"
               (let [oids (post-200-oids defaultKoulutusOids (str "?nimi=" (enc "tietojenkäsittelytieteen perusopinnot")))]
                 (is (= [koulutusOid2 koulutusOid4 koulutusOid5] oids))))))
